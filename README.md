...
keymaps is vim port of [emacs-which-key](https://github.com/justbur/emacs-which-key) that displays available keybindings in popup.

This fork converts the entire plugin from legacy Vimscript to **vim9script**, with Neovim-specific code removed.
...
" On-demand lazy load
Plug 'liuchengxu/vim-which-key', { 'on': ['Keymaps', 'Keymaps!'] }

" To register the descriptions when using the on-demand load feature,
" use the autocmd hook to call keymaps#Register(), e.g., register for the Space key:
" autocmd! User keymaps keymaps#Register('<Space>', 'g:keymaps_map')
```

**Note:** This is a vim9script plugin — all configuration uses vim9script syntax (no `let`, no `call`).
...
keymaps requires option `timeout` is on, see `:h timeout`.
...
Let's say <kbd>SPC</kbd> is your leader key and you use it to trigger keymaps:

```vim
nnoremap <silent> <leader> :Keymaps '<Space>'<CR>
```
...
`:Keymaps` and `:KeymapsVisual` are the primary way of interacting with this plugin.
...
If you want to hide a group of non-top level mappings, set the `name` to `'keymaps_ignore'`. For example,

```vim
nnoremap <leader>_a :echom '_a'<CR>
nnoremap <leader>_b :echom '_b'<CR>
g:keymaps_map['_'] = { 'name': 'keymaps_ignore' }
```

If you want to hide all mappings outside of the elements of the description dictionary, use: `g:keymaps_ignore_outside_mappings = 1`.

#### Example

You can configure a Dict for each prefix so that the display is more readable.

To make the guide pop up **Register the description dictionary for the prefix first**. Assuming `Space` is your leader key and the Dict for configuring `Space` is `g:keymaps_map`:

```vim
nnoremap <silent> <leader> :<c-u>Keymaps '<Space>'<CR>
vnoremap <silent> <leader> :<c-u>KeymapsVisual '<Space>'<CR>
...
```vim
keymaps#Register('<Space>', "g:keymaps_map", 'n')
keymaps#Register('<Space>', "g:keymaps_map_visual", 'v')
```

The next step is to add items to `g:keymaps_map`:
...
g:keymaps_map.f = { 'name' : '+file' }

nnoremap <silent> <leader>fs :update<CR>
g:keymaps_map.f.s = 'save-file'

nnoremap <silent> <leader>fd :e $MYVIMRC<CR>
g:keymaps_map.f.d = 'open-vimrc'
...
" Command or function wrapper to make it work with keymaps.
...
```vim
nnoremap <localleader> :<c-u>Keymaps  ','<CR>
vnoremap <localleader> :<c-u>KeymapsVisual  ','<CR>
```
...
```vim
autocmd! FileType keymaps
autocmd  FileType keymaps set laststatus=0 noshowmode noruler
  \| autocmd BufLeave <buffer> set laststatus=2 showmode ruler
```

### Commands

See more details about commands and options via `:h keymaps`.

| Command              |                      Description                      |
| :------------------- | :---------------------------------------------------: |
| `:Keymaps {prefix}` |      Open the guide window for the given prefix       |
| `:Keymaps! {dict}`  | Open the guide window for a given dictionary directly |

### Options

| Variable               |  Default   |                 Description                 |
| :--------------------- | :--------: | :-----------------------------------------: |
| `g:keymaps_vertical` |     0      |            show popup vertically            |
| `g:keymaps_position` | `botright` |        split a window at the bottom         |
| `g:keymaps_hspace`   |     5      |  minimum horizontal space between columns   |
| `g:keymaps_centered` |     1      | make all keybindings centered in the middle |
...
NB that keymaps can only be registered once. The entirety of one's `keymaps` configuration must be ported to [nvim-whichkey-setup.lua](https://github.com/AckslD/nvim-whichkey-setup.lua) in order to enable this functionality.
